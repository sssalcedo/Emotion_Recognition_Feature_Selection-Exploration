...
setwd ("C:/Users/jesus/Desktop/AMSTERDAM/psychology/Feature-Selection/tsne_python/tsne_python/code")
gemaps_88 <- read_excel("C:/Users/jesus/Desktop/AMSTERDAM/psychology/Feature-Selection/tsne_python/tsne_python/code/eGeMAPSv01b_acoustics_dutch.xlsx")

#---------------Different contexts (GeMaps88)------------------------------
#normalisation
gemaps_88[, c(3:89)] <- sapply(gemaps_88[, c(3:89)], as.numeric)
c_matrix <- as.matrix(gemaps_88 [,3:89])
X <- normalize_input(c_matrix)
colMeans(X)
range(X)
X
#Rtsne dimensions
tsne_out <- Rtsne(X, pca = FALSE,check_duplicates = FALSE, dims=3, perplexity=30,theta=0.5, verbose=TRUE, max_iter = 5000)
