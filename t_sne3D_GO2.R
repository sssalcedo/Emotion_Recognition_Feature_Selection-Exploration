...
head(X)
#Rtsne dimensions
tsne_out <- Rtsne(X, pca = TRUE,check_duplicates = FALSE, dims=3, perplexity=40,theta=0.5, verbose=TRUE, max_iter = 5000)
colors = c( 'tenderness' = 'chartreuse','excitement' =  'blue1')
...
legend("bottomright",c( 'tenderness','excitement'), fill = c('chartreuse','blue1'), cex = 0.7)
dev.off()
...
legend("topright",  c( 'tenderness','excitement'), 
       fill = c('chartreuse','blue1'), cex = 0.8)
dev.off()


#print the dimensions together with general context
d_tsne = as.data.frame(tsne_out$Y)
write.csv(d_tsne,"C:/Users/jesus/Desktop/AMSTERDAM/psychology/Feature-Selection/tsne_python/tsne_python/code/d_tsne2.csv", row.names = FALSE)
...
library(plotly)
fig <- plot_ly(d_tsne2, x=d_tsne2$V1,y=d_tsne2$V2,z=d_tsne2$V3, color = ~d_tsne2$Emotion_Category, text=gemaps_88$Emotion_Category,colors =c('chocolate4', 'yellow','aquamarine','springgreen','cornsilk4', 'blue1', 'deepskyblue2','triumph' = 'bisque2', 'darkorange2'))
fig <- fig %>% add_markers()
...
legend("topright", c( 'respected','positivesurprise' ,'amusement','admiration','lust', 'moved','excitement','triumph','determination'), 
       fill = c('chocolate4', 'yellow','aquamarine','springgreen','cornsilk4', 'blue1', 'deepskyblue2','triumph' = 'bisque2', 'darkorange2'), cex = 0.8)
...
legend3d("topright", c( 'respected','positivesurprise' ,'amusement','admiration','lust', 'moved','excitement','triumph','determination'), fill = c('chocolate4', 'yellow','aquamarine','springgreen','cornsilk4', 'blue1', 'deepskyblue2','triumph' = 'bisque2', 'darkorange2'),cex=1.5, inset=c(0), pch=16)
